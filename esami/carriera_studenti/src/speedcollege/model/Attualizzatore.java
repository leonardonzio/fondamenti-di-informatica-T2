...
public class Attualizzatore {
	...
	public static BiFunction<Esame,LocalDate,Double> UN_VOTO_IN_MENO_AL_MESE =
			(esame, localdate) -> {
				...
				long mesi = period.toTotalMonths();
				double votoNew = esame.getVotoIniziale().getValue().getAsInt() - mesi;
				return (votoNew < 18) ? 18 : votoNew;
			};
	...
	private String toStringVotoPesato(Esame e) {
		if (e.getVotoIniziale().ordinal()<Voto.DICIOTTO.ordinal())
			return e.toString();
		else
